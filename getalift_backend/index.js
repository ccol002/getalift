...
router.get("/passenger/alert/withoutDateVerification/:driverId", function(req, res){
	db_con.query("SELECT DISTINCT passenger.*, passager.username From User passager,User conducteur,Route route,Ride ride,Passenger passenger, RouteDate routeDate Where ride.route = route.id and passenger.ride = ride.id and conducteur.id = route.driver and passenger.passenger = passager.id and conducteur.id = ? and passenger.inTheCar = 0 and routeDate.route = route.id", [req.params.driverId], function(err, result){
		if(err) throw err;
		res.json(result);
	});
});
